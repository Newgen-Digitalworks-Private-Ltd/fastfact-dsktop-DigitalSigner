...
namespace iTextSharpSign
{
    ...
    class PDFSigner
    {
        ...
        public void Sign(string SigReason, string SigContact, string SigLocation, bool visible)
        {
            ...
            st.MoreInfo = (Dictionary<string, string>)this.metadata.getMetaData();
            st.XmpMetadata = this.metadata.getStreamedMetaData();
            PdfSignatureAppearance sap = st.SignatureAppearance;

            PdfSignature dic = new PdfSignature(PdfName.ADOBE_PPKLITE, PdfName.ADBE_PKCS7_DETACHED);
            dic.Date = new PdfDate(sap.SignDate);
            dic.Name = CertificateInfo.GetSubjectFields(this.myCert.Chain[0]).GetField("CN");
            sap.CryptoDictionary = dic;
            sap.Certificate = this.myCert.Chain[0];
            ...
            if (visible)sap.SetVisibleSignature(new iTextSharp.text.Rectangle(100, 100, 250, 150), 1, null);
            sap.PreClose(exc);
            //sap.Close(dic);
            st.Dispose();
            st.Close();
